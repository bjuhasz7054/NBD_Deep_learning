...
class DataCollector:
    ...
    def download_dataset(self):
        if not self.overwrite and os.listdir(self.output_folder):
            self.logger.warning(
                f"dataset folder {self.output_folder} not empty, download"
                " failed"
            )
            return
        ...
        gdown.download(
            id=self.DATASET_ID,
            output=os.path.join(self.output_folder, "dataset.zip"),
        )
        gdown.download(
            id=self.LABELS_TRAIN_ID,
            output=os.path.join(self.output_folder, "labels_train.csv"),
        )
        gdown.download(
            id=self.LABELS_VALIDATE_ID,
            output=os.path.join(self.output_folder, "labels_validate.csv"),
        )
    ...
    def extract_dataset(self):
        ...
        if not os.path.exists(dataset_file):
            self.logger.warning(
                f"dataset file {dataset_file} not found, extraction failed"
            )
            return
